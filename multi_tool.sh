...
while test $# -gt 0; do
	...
	-h|--help)
		...
		echo -e "  -gu TEXT           Grafana panel user (default is ${C_LGn}${grafana_user}${RES})"
		echo -e "  -gp TEXT           Grafana panel password (default is ${C_LGn}${grafana_password}${RES})"
		echo -e "  -i,  --influxdb    install InfluxDB"
		echo -e "  -ap TEXT           InfluxDB administrator password (default is ${C_LGn}generated${RES})"
		echo -e "  -rp TEXT           InfluxDB reader password (default is ${C_LGn}generated${RES})"
		echo -e "  -t,  --telegraf    install Telegraf"
		echo -e "  -sn TEXT           server name for Telegraf config"
		echo -e "  -iu URL            InfluxDB URL to connect Telegraf (default is ${C_LGn}${influxdb_url}${RES})"
		echo -e "  -id                import Grafana dashboard from ${json_file} or from URL"
		echo -e "  -ju URL            raw JSON dashboard URL"
