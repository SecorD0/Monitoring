...
main() {
	...
	if [ "$previous_epoch_info" != "null" ]; then
		...
		local q_solana_price=`printf "%f" $(jq ".[0][4]" <<< "$previous_epoch_info")`
		local q_stake_reward=`printf "%f" $(jq ".[0][5]" <<< "$previous_epoch_info")`
		...
		if (( `bc -l <<< "$q_stake_reward == 0"` )); then
			local q_stake_reward=`timeout 60s $daemon vote-account $vote_address --with-rewards 2>/dev/null | grep -A2 "Epoch Rewards:" | tail -1 | awk '{print $3}' | grep -oE '[0-9]+.[0-9]+'`
